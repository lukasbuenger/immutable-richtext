...
const create = (selection, blocks) => {
    if (!selection || !blocks) {
        throw new Error('Invalid EditorState detected!');
    }
    ...
    (editorStateToPaste, editorState) =>
        R.compose(
            ...
            state =>
                setBlockList(
                    BlockList.copySelection(
                        Selection.setStart(0, 0, getSelection(editorState)),
                        getBlockList(editorState),
                    ),
                    state,
                ),
    ...
    (text, editorState) => {
        ...
        return create(
            Selection.collapseTo(
                ...
                Selection.getEndOffset(selection) + text.length,
                selection,
            ),
...
const removeCharBeforeSelection = (editorState) => {
    ...
    return create(
        R.cond([
            [
                R.flip(Selection.isCollapsedAtBlockListStart),
                R.always(selection),
            ],
            [
                R.flip(Selection.isCollapsedAtBlockStart),
...
const getFormatNamesInSelection = editorState =>
    BlockList.reduceSpansInSelection(
        true,
        (acc, span) => acc.union(Span.getFormatNames(span)),
        Set(),
        getSelection(editorState),
        getBlockList(editorState),
    );
...
const append = R.curry(
    (mergeEdges, editorStateToAppend, editorState) =>
        create(
...
const prepend = R.curry(
    (mergeEdges, editorStateToPrepend, editorState) =>
        append(mergeEdges, editorState, editorStateToPrepend),
);
