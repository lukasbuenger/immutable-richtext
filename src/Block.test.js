...
test('Block.isEmpty', assert => {
  ...
  assert.equal(
    Block.isEmpty(Block.create()),
    true,
    'should return true if you pass it an empty block'
  )

  assert.equal(
    Block.isEmpty(Block.createWithText('foo')),
    false,
...
test('Block.removeRange', assert => {
  ...
  assert.equal(
    Block.getBlockText(
      Block.removeRange([6, 12], fixtures)
    ),
  ...
  assert.deepEqual(
    I.map(
      Span.asRange,
      Block.getSpanList(
        Block.removeRange([2, 21], fixtures)
      )
    ).toJS(),
    [[0, 3], [3, 8]],
...
test('Block.append', assert => {
  ...
  const fixtures = getFixtures()
  const fixturesToAppend = Block.createWithText(
...
test('Block.prepend', assert => {
  ...
  const fixtures = getFixtures()
  const fixturesToPrepend = Block.createWithText(
    'Consectetur adipiscing elit.'
  )
  ...
  assert.deepEqual(
    ...
    I.map(
      Span.asRange,
      Block.getSpanList(
        Block.append(fixtures, fixturesToPrepend)
      )
    ).toJS(),
