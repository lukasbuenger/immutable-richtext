...
const removeReducer = range => (newSpanList, span) => {
    ...
    } else if (Range.isInsideOrEqual(range, spanRange)) {
        return newSpanList;
    } else if (Range.containsOrEqual(range, spanRange)) {
        return I.push(
            Span.offsetEnd(
                -1 * Range.getLength(range),
                span,
    ...
    } else if (Range.intersectsStart(range, spanRange)) {
        return I.push(
            Span.setEnd(
    ...
    } else if (Range.intersectsEnd(range, spanRange)) {
        return I.push(
            Span.setStart(
                Range.getStart(range),
                Span.offsetEnd(
...
const copyReducer = range => (newSpanList, span) => {
    const spanRange = Span.asRange(span);

    if (Range.isBefore(range, spanRange) || Range.isAfter(range, spanRange)) {
        return newSpanList;
    } else if (Range.isInsideOrEqual(range, spanRange)) {
        return I.push(
            Span.offset(
        ...
        return I.push(
            Span.setStart(
                Span.getStart(span) - Range.getStart(range),
                Span.setEnd(
                    Range.getLength(range),
                    span,
                ),
...
const copyRange = R.curry(
    (range, spanList) =>
        I.reduce(copyReducer(range), List(), spanList),
);

const splitReducer = range => (newSpanList, span) => {
    const spanRange = Span.asRange(span);
...
const splitAtRange = R.curry(
    (range, spanList) =>
        I.reduce(splitReducer(range), List(), spanList),
);

const offsetAll = R.curry(
    (offset, spanList) => I.map(Span.offset(offset), spanList),
);

const offsetMapper = (offset, range, lastContainingSpan) => (span) => {
    const spanRange = Span.asRange(span);
    ...
    (offset, range, spanList) =>
        I.map(
            offsetMapper(
                ...
                I.findLast(
                    span =>
                        Range.containsOrEqual(range, Span.asRange(span)),
                    spanList,
                ),
            ),
            spanList,
...
const mapInRange = R.curry(
    (lazy, mapperFn, range, spanList) =>
        I.map(
            (span) => {
                return isInRange(lazy, range, Span.asRange(span))
                    ? mapperFn(span)
                    : span;
            },
            spanList,
...
const reduceInRange = R.curry(
    (lazy, reducerFn, initialReduction, range, spanList) =>
        I.reduce(
            (reduction, span) => {
                return isInRange(lazy, range, Span.asRange(span))
...
const normalizeReduzer = (newSpanList, span) => {
    ...
    if (Span.isEmpty(span)) {
        return newSpanList;
    } else if (!previousSpan || !Span.haveSameFormats(previousSpan, span)) {
        return I.push(
            span,
            newSpanList,
        );
