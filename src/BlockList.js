...
const getText = I.reduce(
  (text, block) =>
    text.concat(Block.getBlockText(block)),
  ''
)
...
const isEmpty = blockList =>
  getSize(blockList) === 1 &&
  Block.isEmpty(getFirst(blockList))

const removeReducer = selection => (
  newBlockList,
  block,
  blockIndex
) => {
  ...
  return I.push(block, newBlockList)
}

const removeSelection = R.curry(
  (selection, blockList) =>
    I.reduce(
      removeReducer(selection),
      List(),
      blockList
    )
)

const copyReducer = selection => (
  newBlockList,
  block,
  blockIndex
) => {
...
const copySelection = R.curry(
  (selection, blockList) =>
    I.reduce(
      copyReducer(selection),
      List(),
      blockList
    )
)

const splitReducer = selection => (
  newBlockList,
  block,
  blockIndex
) => {
...
const mapSpansInSelection = R.curry(
  (lazy, mapperFn, selection, blockList) =>
    I.reduce(
      mapReducer(lazy, mapperFn, selection),
      List(),
      blockList
    )
)
...
const reduceSpansInSelection = R.curry(
  (
    ...
    I.reduce(
      reduceReducer(lazy, reducerFn, selection),
      initialReduction,
      blockList
    )
)
  ...
  (selection, blockList) => {
    if (
      Selection.isCollapsedAt(
        ...
        Block.getLength(getLast(blockList)),
...
const fromJS = obj => List(obj.map(Block.fromJS))
