...
const offsetStart = R.curry((delta, span) =>
  I.update('start', x => x + delta, span)
)

const offsetEnd = R.curry((delta, span) =>
  I.update('end', x => x + delta, span)
)

const offset = R.curry((delta, span) =>
  R.compose(
    offsetStart(delta),
    offsetEnd(delta)
  )(span)
...
const isEmpty = span => getLength(span) === 0
...
const haveSameFormats = R.curryN(
  2,
  R.useWith(I.is, [
    getFormatNames,
    getFormatNames
...
const addFormat = R.curryN(
  2,
  R.compose(
    R.apply(I.setIn),
    R.useWith(R.concat, [
      format => [
        getPathFromFormatRecord(format),
        format
      ],
      R.of
    ])
...
const removeFormat = R.curryN(
  2,
  R.useWith(I.removeIn, [
    getPathFromFormatName,
    R.identity
  ])
)
