...
test('SpanList.removeRange', (assert) => {
    ...
    assert.equal(
        SpanList.getFirst(SpanList.removeRange([6, 12])(fixtures)),
        SpanList.getFirst(fixtures),
    ...
    assert.deepEqual(
        Span.asRange(
            SpanList.getLast(SpanList.removeRange([6, 12], fixtures)),
        ),
        [7, 9],
    ...
    assert.equal(
        SpanList.getSize(SpanList.removeRange([3, 12])(fixtures)),
        3,
    ...
    assert.deepEqual(
        SpanList.removeRange([2, 11], fixtures).toJS(),
        [
            { start: 0, end: 2, formats: {} },
            { start: 2, end: 4, formats: { i: { name: 'i', data: {} } } },
...
test('SpanList.copyRange', (assert) => {
    ...
    assert.deepEqual(
        ...
        [
            { start: 0, end: 2, formats: {} },
            { start: 2, end: 7, formats: { i: { name: 'i', data: {} }, b: { name: 'b', data: {} } } },
            { start: 7, end: 9, formats: { i: { name: 'i', data: {} } } },
        ],
    );
});
...
test('SpanList.splitAtRange', (assert) => {
    ...
    assert.deepEqual(
        SpanList.splitAtRange([2, 11], fixtures).toJS(),
        [
            { start: 0, end: 2, formats: {} },
            { start: 2, end: 4, formats: {} },
            { start: 4, end: 9, formats: { i: { name: 'i', data: {} }, b: { name: 'b', data: {} } } },
            { start: 9, end: 11, formats: { i: { name: 'i', data: {} } } },
            { start: 11, end: 13, formats: { i: { name: 'i', data: {} } } },
...
test('SpanList.offsetAll', (assert) => {
    ...
    assert.deepEqual(
        ...
        [
            ...
            { start: 6, end: 11, formats: { i: { name: 'i', data: {} }, b: { name: 'b', data: {} } } },
            { start: 11, end: 15, formats: { i: { name: 'i', data: {} } } },
...
test('SpanList.mapInRange', (assert) => {
    ...
    const mapper = span => Span.addFormat(format, span);

    assert.equal(
        SpanList.getFirst(
            SpanList.mapInRange(true, mapper, [5, 8], fixtures),
        ),
        SpanList.getFirst(fixtures),
    ...
    assert.equal(
        SpanList.getAtIndex(
            1,
            SpanList.mapInRange(false, mapper, [5, 10], fixtures),
        ),
        SpanList.getAtIndex(1, fixtures),
    ...
    assert.equal(
        Span.hasFormat(
            ...
            SpanList.getAtIndex(
                1,
                SpanList.mapInRange(true, mapper, [5, 10], fixtures),
            ),
        ),
        true,
    ...
    assert.equal(
        SpanList.getAtIndex(
            2,
            SpanList.mapInRange(false, mapper, [5, 8], fixtures),
        ),
        SpanList.getAtIndex(2, fixtures),
...
test('SpanList.reduceInRange', (assert) => {
    ...
    const reducer = (acc, span) => acc.concat(Span.getFormatNames(span));
...
test('SpanList.offsetAfterRange', (assert) => {
    ...
    assert.equal(
        SpanList.getFirst(
            SpanList.offsetAfterRange(5, [5, 5], fixtures),
        ),
        SpanList.getFirst(fixtures),
    ...
    assert.equal(
        SpanList.getAtIndex(
            1,
            SpanList.offsetAfterRange(5, [3, 11], fixtures),
        ),
        SpanList.getAtIndex(1, fixtures),
    ...
    assert.equal(
        SpanList.getAtIndex(
            1,
            SpanList.offsetAfterRange(5, [5, 11], fixtures),
        ),
        SpanList.getAtIndex(1, fixtures),
    ...
    assert.deepEqual(
        SpanList.offsetAfterRange(2, [9, 9], fixtures).toJS(),
        [
            { start: 0, end: 4, formats: {} },
            { start: 4, end: 9, formats: { i: { name: 'i', data: {} }, b: { name: 'b', data: {} } } },
            { start: 9, end: 15, formats: { i: { name: 'i', data: {} } } },
            { start: 15, end: 17, formats: {} },
        ],
...
test('SpanList.normalize', (assert) => {
    ...
    assert.deepEqual(
        SpanList.normalize(denormalizedFixtures).toJS(),
        [
            { start: 0, end: 4, formats: {} },
            { start: 4, end: 13, formats: { i: { name: 'i', data: {} } } },
            { start: 13, end: 15, formats: {} },
        ],
...
test('SpanList.prepend', (assert) => {
    ...
    assert.deepEqual(
        SpanList.prepend(listToPrepend)(fixtures).toJS(),
        SpanList.append(fixtures)(listToPrepend).toJS(),
